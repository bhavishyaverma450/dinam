...
export function AssistantPanel({ open, onOpenChange }: AssistantPanelProps) {
        ...
        async (historyIncludingLatestUser: ChatTurn[]) => {
            ...
            for (let round = 0; round < MAX_TOOL_ROUNDS; round += 1) {
                ...
                for (const tc of toolCalls) {
                    ...
                    let payload: unknown
                    try {
                        payload = await executeAgentTool(
                            tc.function.name,
                            parsed,
                            ctx,
                        )
    ...
    return (
        <>
            ...
            <aside
                className={cn(
                    "fixed top-0 right-0 z-50 flex h-dvh w-full max-w-md flex-col border-border/60 border-l bg-background shadow-2xl",
                )}
                role="dialog"
                aria-modal="true"
            ...
            >
                ...
                >
                    {turns.length === 0 && !busy ? (
                        <p className="text-sm text-muted-foreground">
                            Ask to manage tasks, bookmarks, quick links, theme,
                            or use browser tools (clipboard, speak, open URLs,
                            …). Set chat API URL, token, and model under
                ...
                {error ? (
                    <p className="shrink-0 border-border/40 border-t px-4 py-2 text-destructive text-xs">
                        {error}
                    </p>
                ) : null}

                <form
                    className="shrink-0 border-border/60 border-t p-3"
                    onSubmit={(e) => {
                        e.preventDefault()
                        void send()
                    }}
